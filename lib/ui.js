 ...
 */
async function singleSelect(title, items) {
  return new Promise((resolve) => {
    let cursor = 0;
    ...
    function draw() {
      console.log(`  ${BOLD}${COLORS.white}${title}${RESET}`);
      console.log(`  ${COLORS.dimGray}↑/↓ navigate  enter confirm${RESET}`);
      console.log('');

      for (let i = 0; i < items.length; i++) {
        const ptr = i === cursor ? POINTER : POINTER_NONE;
        const dot = i === cursor ? `${COLORS.cyan}●${RESET}` : `${COLORS.gray}○${RESET}`;
        const nameColor = i === cursor ? COLORS.white : COLORS.gray;
        const name = `${BOLD}${nameColor}${items[i].name}${RESET}`;
        const desc = items[i].description ? `  ${COLORS.dimGray}${items[i].description}${RESET}` : '';
        console.log(`  ${ptr} ${dot} ${name}${desc}`);
      }
      console.log('');
    }
